...
microcommands = {"main":[10,"Der Befehl verzweigt zum naechsten Mikroprogramm abhaengig vom Inhalt von MBR"],\
                 ...
                 "goback":[118,"Der Befehl springt um x Befehle zurueck!"],\
                 "declare":[0,"Das SChluesselwort reserviert speicher"]}
...
output.write("9:" + hex(int(CPP/4)+(co)).lstrip("0x").rjust(8,"0")[6:4] + "\n")
...
for i in symboltable["constants"]:
    ...
    output.write(str(CPP+(symboltable["constants"][i][1]*4)) + ":0x" + symbotable["constants"][i][0][2:4] + "\n")
    output.write(str(CPP+(symboltable["constants"][i][1]*4)+1) + ":0x" + symbotable["constants"][i][0][4:6]+ "\n")
    output.write(str(CPP+(symboltable["constants"][i][1]*4)+2) + ":0x" + symbotable["constants"][i][0][6:8]+ "\n")
    output.write(str(CPP+(symboltable["constants"][i][1]*4)+3) + ":0x" + symbotable["constants"][i][0][8:10]+ "\n")
for i in symboltable["variables"]:
    ...
    output.write(str(CPP+((co + symboltable["variables"][i][1])*4)) +":0x" + symboltable["variables"][i][0][2:4]+ "\n")
    output.write(str(CPP+((co + symboltable["variables"][i][1])*4)+1) +":0x" + symboltable["variables"][i][0][4:6]+ "\n")
    output.write(str(CPP+((co + symboltable["variables"][i][1])*4)+2) +":0x" + symboltable["variables"][i][0][6:8]+ "\n")
    output.write(str(CPP+((co +  symboltable["variables"][i][1])*4)+3) +":0x" + symboltable["variables"][i][0][8:10]+ "\n")
# write commands
p = PC
